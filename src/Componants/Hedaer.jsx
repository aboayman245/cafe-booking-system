  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > lastScrollY && window.scrollY > 400) {
        // لو نزلت لأسفل واوصلت بعد مسافة معينة
        setShow(false); // اخفاء الهيدر
      } else {
        setShow(true); // ظهوره عند الصعود
      }
...
{/* اسم اليوزر وي لو  كان عميل ولا ادمين */}
          ...
          <NavLink to="/login">
            <button className="btn btn-ghost btn-sm flex items-center gap-2 text-black hover:bg-gray-200 rounded-2xl border-0">
              <LogOut className="w-4 h-4" />
              <span className="hidden sm:inline">Logout</span>
            </button>
          </NavLink>
